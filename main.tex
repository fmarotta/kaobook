...
\documentclass[
	...
	%draft=true, % If uncommented, images will be replaced by empty boxes VEL: not true, this adds a ruler in the header and footer but images remain
...
%\usepackage{styles/plaintheorems} % VEL: uncommenting this while commenting the above leads to a compilation error about \openbox already being defined. Once that's fixed, it's worth commenting here that only one of them should be uncommented (there's also a compilation error if both are uncommented).
